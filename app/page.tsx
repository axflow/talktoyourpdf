...
import { SHOPIFY_S1_PDF, type PDFType } from '@/lib/pdf';
...
function PDFUploadView(props: { pdf: PDFType }) {
  ...
  async function onUpload() {
    ...
    try {
      // const { body } = await uploadFile('/api/upload', file, () => {});
    } catch (error) {
      console.error(error);
    } finally {
      setUploading(false);
  ...
  return (
    <>
      ...
      ></object>
      <p className="mt-4 text-xs italic text-center">{pdf.filename}</p>
...
function ExplainUploading(props: {}) {
  return (
    <>
      ...
      <p className="mt-6">
        ...
        performant to provide only the most relevant snippets.
      </p>
    ...
    <main className="min-h-screen flex">
      <section className="bg-muted bg-zinc-900 dark:border-r lg:max-w-[600px] pt-[52px]">
        <div className="pl-6 pr-9 pt-24">
          {pdf !== null ? <ExplainUploading /> : <Welcome setPdf={setPdf} />}
        </div>
      </section>
